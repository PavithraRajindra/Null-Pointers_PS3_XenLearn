...
def show_ar_page():
    ...
    with left_col:
        ...
        if st.button("👤 Profile", key="ar_nav_profile"):
            navigate_to('profile')
        st.button("📚 Courses", key="ar_nav_courses")
        if st.button("🏠 Back to Dashboard", key="ar_nav_dashboard"):
            navigate_to('dashboard')
        if st.button("📷 Search with Camera", key="ar_nav_camera"):
            navigate_to('camera')
        if st.button("👥 Collaborate", key="ar_nav_collaborate"):
            navigate_to('collaborate')
        st.button("🏪 Reward Shop", key="ar_nav_shop")
        st.button("🤖 DoubtAI", key="ar_nav_ai")
        st.button("🎓 Certifications", key="ar_nav_cert")
        st.button("⚙️ Settings", key="ar_nav_settings")
    
    # Main content
    with main_col:
        st.write("Click below to start AR object detection.")
        
        if st.button("Start AR Detection", key="start_ar_button"):
            # Use the working object detection function
            ...
            if detected_objects:
                ...
                if "cell phone" in detected_objects:
                    ...
                    st.write("Displaying 3D Model of Cell Phone")
                    st.components.v1.iframe("https://tinyglb.com/viewer/896e639849ab4069a3ad4dd2f9c2823c", height=500)
                elif "battery" in detected_objects:
                    st.write("Displaying 3D Model of Cell Phone")
                    st.components.v1.iframe("https://tinyglb.com/viewer/bed08d2353994428836aac143afed308 ", height=500)
                    
            else:
                st.write("No objects detected. Please try again.")
    ...
    with right_col:
        ...
        st.markdown("""
            <div style='background-color: white; padding: 15px; border-radius: 10px; margin: 10px 0;'>
                <h4 style='color: black;'>How to use AR Learning</h4>
                <p style='color: black;'>1. Point your camera at an object</p>
                <p style='color: black;'>2. Click "Start AR Detection"</p>
                <p style='color: black;'>3. Explore the 3D model</p>
                <p style='color: black;'>4. Learn about its components</p>
            </div>
        """, unsafe_allow_html=True)
        
        st.markdown("""
            <div style='background-color: white; padding: 15px; border-radius: 10px; margin: 10px 0;'>
                <h4 style='color: black;'>Supported Objects</h4>
                <p style='color: black;'>• Cell phones</p>
                <p style='color: black;'>• Books</p>
                <p style='color: black;'>• Microscopes</p>
                <p style='color: black;'>• More coming soon!</p>
            </div>
        """, unsafe_allow_html=True)
...
def detect_objects():
    ...
    placeholder.info("Opening camera... Please wait.")
    ...
    # Get detected objects with scores above threshold
    detected_labels = set()
    for label, score in zip(labels, scores):
        if score > 0.7:
            detected_labels.add(class_names[label].lower())
